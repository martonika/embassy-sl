...
///
/// `period` is a 32bit integer, so It overflows on 2^32 * 2^31  seconds of uptime.
...
impl RtcDriver {
    fn init(&'static self, irq_prio: crate::interrupt::Priority) {
        ...
        // Enable compares once and leave them running - can be disabled by grp0_ien()
        r.grp0_ctrl().write(|w| {
            w.set_cmp0en(true);
            w.set_cmp1en(true)
        });
    ...
    fn on_interrupt(&self) {
        ...
        if r.grp0_if().read().ovf() {
            r.grp0_if_clr().write(|w| w.set_ovf(true));
            self.next_period();
        }
        // Compare 0
        if r.grp0_if().read().cmp0() {
            r.grp0_if_clr().write(|w| w.set_cmp0(true));
            self.next_period();
        }

        // Compare 1
        if r.grp0_if().read().cmp1() {
            r.grp0_if_clr().write(|w| w.set_cmp1(true));
    ...
    fn next_period(&self) {
        critical_section::with(|cs| {
            ...
            if at < t + 0xc000_0000 {
                ...
                r.grp0_ien().modify(|w| w.set_cmp1(true));
            }
        })
    }
    ...
    fn set_alarm(&self, cs: CriticalSection, timestamp: u64) -> bool {
        ...
        if timestamp <= t {
            ...
            r.grp0_ien().modify(|w| w.set_cmp1(false));
        ...
        r.grp0_ien().modify(|w| w.set_cmp1(diff < 0xc000_0000));
